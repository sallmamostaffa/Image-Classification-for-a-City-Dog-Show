...
# REVISED DATE: 
# PURPOSE: Create a function get_pet_labels that creates the pet labels from 
...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           filename_words = in_files[idx].lower().split('_')
           
           # Extracts pet label from filename by ignoring words containing digits
           for word in filename_words:
               if word.isalpha():
                   pet_label += word + " "
           
           # Strips off trailing whitespace characters
